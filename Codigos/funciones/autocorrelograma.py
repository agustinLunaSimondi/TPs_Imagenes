...
def autocorrelograma_rgb(imagenes, sufijos, distancias=[1, 3, 5], n_bins=64, output_dir='./output/autocorrelogramas'):
    """
    ...
    for i, imagen in enumerate(imagenes):
        if imagen is None:
            ...
            for k, d in enumerate(distancias):
                ...
                for x in range(imagen_cuantizada.shape[0]):
                    for y in range(imagen_cuantizada.shape[1]):
                        ...
                        for dx in range(-d, d + 1):
                            for dy in range(-d, d + 1):
                                # Evitar el mismo píxel (dx == 0 y dy == 0)
                                if dx == 0 and dy == 0:
                                    continue
                                nx, ny = x + dx, y + dy
            ...
            plt.figure(figsize=(15, 5))
            for k, d in enumerate(distancias):
                #autocorrelograma_rgb = np.sum(autocorrelograma_total[:, :, :, k], axis=(0, 1, 2))  # Sumamos en 3D
